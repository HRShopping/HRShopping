...
public class ActivityFindAllOrderPresenter implements IActivityFindAllOrderBiz {
        ...
        observable.subscribeOn(Schedulers.io())
                ...
                .subscribe(new Observer<orderList>() {
                    ...
                    public void onNext(orderList orderList) {
                        Log.d("111", "onNext: " + orderList.getOrderList().get(0).getSorderSet().get(0).getPrice());
                        iActivityFindAllOrder.getAllOrder(orderList);
                        if (flag == "cancel") {
    ...
    public void deleteOrder(String id) {
        Observable<Response> observable = HttpMethods.getInstance().findAllOrderService.deleteOrder(id);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
...
//                        iActivityFindAllOrder.onSuccessDelete();
    ...
    public void cancelOrder(String id) {
        Observable<Response> observable = HttpMethods.getInstance().getFindAllOrderService().cancelOrder(id);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<Response>() {
                    @Override
